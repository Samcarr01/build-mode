...
allowed-tools: Bash(npm run build) Bash(npm run lint) Bash(npx tsc --noEmit) Bash(git add *) Bash(git commit *)
# Generated by build-mode. Keep the checks in section 1 - they are the point of this file.
---

Four steps, in order. Evidence first, paperwork second, and the push is a decision rather
than a reflex.
...
`git add -A` and commit with the task ID in the message (`M1-T3: sign-up form`).
...
## 4. Then decide about pushing, and default to not

Read the **Deploying** section of `CLAUDE.md` before you push anything.

- **A push that does not deploy** - a branch, or deploys triggered by hand - just push,
  and say you have.
...
and whether it is pushed or waiting on the user.
```

Three things not to undo later. **Checks before paperwork**, or Claude Code writes an
optimistic entry and then discovers the build is broken. **The commit is not optional,
the push is** - committing every task keeps the history and the SHA anchor; pushing is a
judgement call that belongs to the user whenever it puts something in front of real
people. **The commit SHA** is the difference between "revert to before M1-T3" being a
command and being archaeology.

Getting that backwards is the expensive mistake. A `/checkpoint` that pushes by default
turns every finished task into a deploy, and on a project where migrations run
pre-deploy it means a schema change against live data while nobody is watching.
Committing costs nothing and is always recoverable. Pushing sometimes is not.
...
- **`git push` is deliberately not in the allow list.** On a project where a push deploys, that prompt is the last gate before production. Leave it asking.
